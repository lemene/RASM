...
use strict;
use 5.010;
my $fa = shift;	# my $fa = shift; 的作用是将脚本运行时传递的第一个参数（通常是文件名或路径）赋值给变量 $fa
my $fai = "$fa.fai";
if (! -e $fai) {
	die '[Error] Can\'t find the fai file. You can run "samtools faidx" to generate a fai file.'
}
# 在脚本开始部分，打开一个输出文件句柄
open my $output_fh, '>', 'mis.bed' or die "Cannot open output file: $!";
...
while (<POSI>) {
	...
	if ( $type eq 'expanded_3' ) {
		...
		print $output_fh join("\t",$ctg,$posi_ref,$posi_ref+$length+(length $transition)+(length $repeat_var),$id_seq,$type,"ref",$length), "\n";
		print $output_fh join("\t",$ctg,$posi_simu,$posi_simu+$length+(length $transition)*2+(length $repeat_var)*2,$id_seq,$type,"simu",$length,$posi_simu+$length+(length $transition)+(length $repeat_var),$posi_simu+$length+(length $transition)*2+(length $repeat_var)), "\n";
	...
	if ( $type eq 'expanded_4' ) {
		...
		say join("\t",$id_seq,2,$ctg,$type,$length,$posi_simu,$posi_simu+$length*2+(length $transition)*3+(length $repeat_var)*2,
		              $posi_simu+$length+(length $transition)+(length $repeat_var),$posi_simu+$length+(length $transition)*2+(length $repeat_var));
		print $output_fh join("\t",$ctg,$posi_ref,$posi_ref+$length+(length $transition)+(length $repeat_var),$id_seq,$type,"ref",$length), "\n";
		print $output_fh join("\t",$ctg,$posi_simu,$posi_simu+$length*2+(length $transition)*3+(length $repeat_var)*2,$id_seq,$type,"simu",$length,$posi_simu+$length+(length $transition)+(length $repeat_var),$posi_simu+$length+(length $transition)*2+(length $repeat_var)), "\n";
		$ref_adjust += length $repeat_var;
	...
	if ( $type eq 'collasped' ) {
		...
		print $output_fh join("\t",$ctg,$posi_ref,$posi_ref+$length+(length $transition)+(length $repeat_var),$id_seq,$type,"ref",$length), "\n";
		print $output_fh join("\t",$ctg,$posi_simu,$posi_simu+$length,$id_seq,$type,"simu",$length,$posi_simu,$posi_simu+$length), "\n";
	...
	if ( $type eq 'inv' ) {
		...
		print $output_fh join("\t",$ctg,$posi_ref,$posi_ref+$length+$trans_len+(length $repeat_var),$id_seq,$type,"ref",$length,$trans_len), "\n";
		print $output_fh join("\t",$ctg,$posi_simu,$posi_simu+$length+$trans_len+(length $repeat_var),$id_seq,$type,"simu",$length,$trans_len,$posi_simu+(length $repeat_var),$posi_simu+(length $repeat_var)+$trans_len), "\n";
		#
		$ref_adjust += length $repeat_var;
	...
	if ( $type eq 'ins' ) {
		...
		print $output_fh join("\t",$ctg,$posi_ref,$posi_ref+1,$id_seq,$type,"ref",$length), "\n";
		print $output_fh join("\t",$ctg,$posi_simu,$posi_simu+$length,$id_seq,$type,"simu",$length), "\n";
		#
	...
	if ( $type eq 'del' ) {
		...
		print $output_fh join("\t",$ctg,$posi_ref,$posi_ref+1,$id_seq,$type,"ref",$length), "\n";
		print $output_fh join("\t",$ctg,$posi_simu,$posi_simu+1,$id_seq,$type,"simu",$length), "\n";
		#
		$simu_adjust -= $length;
	}

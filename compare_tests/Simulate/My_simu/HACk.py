...
def write_bed(bed_ls, bed_out):
	with open(bed_out, "a") as fo:
		for el in bed_ls:
			fo.write("\t".join(map(str, el)) + "\n")
			# fo.write("\t".join())

def HapMaker(pyref,pychroms,hapdict,hapfout,old_Modified_bed,new_Modified_bed):
	...
	'''cnt = 0
	for chr, val in hapdict.items():
		alts = hapdict[chr]
		cnt += len(val)
		print(chr, ": ", len(val))
	print("Sum SV num: ", cnt)
	return'''

	for chrs in pychroms:	# 遍历染色体
		...
		if chrs not in hapdict.keys(): #this means that it has not to be modified
			...
			new_bed_ls = []
			old_bed_ls = [[chrs] + list(x) for x in altsfltrd]
			i=0
			new_l, new_r = 0, 0
			while i < len(altsfltrd):	# 遍历所有的SV
				alt = altsfltrd[i]
				# print(alt)
				...
				'''变异结束，写入bed_ls'''
				# print("alt: ",alt)	# alt:  (657361, 657824, 'deletion', 'None', 'TCGTGG')
				new_r = len(final_seq) + 1
				new_alt = list(alt)
				new_alt[0] = new_l
				new_alt[1] = new_r
				# print("new_alt: ",new_alt)
				new_bed_ls.append([chrs] + new_alt)
				# 非变异序列
				...
				i+=1
		# if chrs in hapdict.keys() and len(new_bed_ls) > len(hapdict[chrs]):
		# 	print(chrs, "over!!!")
		# elif chrs not in hapdict.keys():
		# 	if len(old_bed_ls) > 0:
		# 		print("Error,", chrs)
		# 		print(len(old_bed_ls))
		if chrs in hapdict.keys(): #this means that it has been modified
			write_bed(old_bed_ls, old_Modified_bed)
			write_bed(new_bed_ls, new_Modified_bed)
...
def run(parser,args):
	...
	for dicts in d.keys():	# dicts: hp1、hp2...

		now=datetime.now().strftime('%d/%m/%Y %H:%M:%S')
		print('[' + now + '][Message] Generating haplotype ' + dicts)
		hapout=os.path.abspath(c.OUT + '/' + dicts + '.fa')
		# Add: 
		old_Modified_bed = os.path.abspath(c.OUT + '/' + dicts + '_old.bed')
		new_Modified_bed = os.path.abspath(c.OUT + '/' + dicts + '_new.bed')
		# 
		HapMaker(ref,chrs,d[dicts],hapout,old_Modified_bed,new_Modified_bed)
		now=datetime.now().strftime('%d/%m/%Y %H:%M:%S')
...
if __name__ =='__main__':
	import argparse
	parser = argparse.ArgumentParser(prog='VISOR', description='''VarIants SimulatOR''') 
